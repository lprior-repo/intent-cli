...
pub fn load_from_string(content: String) -> Result(Dict(String, String), AnswerLoaderError) {
  ...
  case string.contains(lower, "answers") {
    ...
    True -> {
      ...
      case find_brace_block(content, "answers:") {
        Ok(block) -> Ok(block)
        Error(Nil) -> Error("Malformed answers block: missing or unclosed braces")
      }
    }
  }
}
...
fn parse_answer_entries(block: String) -> Result(Dict(String, String), AnswerLoaderError) {
  // Split by lines and parse key: "value" pairs
  block
  |> string.split("\n")
  |> list.map(string.trim)
  |> list.filter(fn(line) { !string.is_empty(line) && !string.starts_with(line, "//") })
            ...
            case validate_question_id(key) {
              True -> {
                case string.is_empty(string.trim(value)) {
                  True -> Error(SchemaError(
                    "Empty answer for question: " <> key,
                  ))
                  False -> Ok(dict.insert(dict_so_far, key, string.trim(value)))
                }
              }
              False -> Error(SchemaError(
                "Invalid question ID: " <> key <> " (must be alphanumeric + underscore)",
              ))
            }
          }
