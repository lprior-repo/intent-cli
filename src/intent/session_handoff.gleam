...
pub fn calculate_mental_model_indicators(
  ...
  let mut_dict = case session.rounds_completed {
    ...
    _ -> dict.insert(mut_dict, "round_1_status", "Pending")
  }
  ...
  let mut_dict = case session.rounds_completed {
    ...
    _ -> dict.insert(mut_dict, "round_2_status", "Pending")
  }
  ...
  let mut_dict = case session.rounds_completed {
    ...
    _ -> dict.insert(mut_dict, "round_3_status", "Pending")
  }
  ...
  let mut_dict = case session.rounds_completed {
    ...
    _ -> dict.insert(mut_dict, "round_4_status", "Pending")
  }
  ...
  let mut_dict = case session.rounds_completed {
    ...
    _ -> dict.insert(mut_dict, "round_5_status", "Pending")
  }
...
pub fn extract_key_assumptions(session: InterviewSession) -> List(String) {
  ...
  let conflict_assumptions =
    list.filter_map(session.conflicts, fn(conflict) {
      case conflict.chosen {
        -1 -> Ok("Unresolved conflict: " <> conflict.description)
        _ -> Error(Nil)
      }
    })
  ...
  let stage_assumption = case session.stage {
    ...
    interview.Complete -> "Interview marked as Complete"
    interview.Paused -> "Interview was paused mid-session"
  }
...
fn handoff_decoder() -> dynamic.Decoder(SessionHandoff) {
  fn(dyn) {
    ...
    use handoff_id <- result.try(dynamic.field("handoff_id", dynamic.string)(
      dyn,
    ))
    ...
    use pause_reason <- result.try(dynamic.field("pause_reason", dynamic.string)(
      dyn,
    ))
    ...
    use current_round <- result.try(dynamic.field("current_round", dynamic.int)(
      dyn,
    ))
    use questions_answered <- result.try(dynamic.field(
      "questions_answered",
      dynamic.int,
    )(dyn))
    use gaps_unresolved <- result.try(dynamic.field(
      "gaps_unresolved",
      dynamic.int,
    )(dyn))
    use conflicts_unresolved <- result.try(dynamic.field(
      "conflicts_unresolved",
      dynamic.int,
    )(dyn))
    use next_question_id <- result.try(dynamic.field(
      "next_question_id",
      dynamic.string,
    )(dyn))
    use mental_model_indicators <- result.try(dynamic.field(
      "mental_model_indicators",
      decode_mental_model,
    )(dyn))
    use key_assumptions <- result.try(dynamic.field(
      "key_assumptions",
      dynamic.list(dynamic.string),
    )(dyn))
    use full_session_jsonl <- result.try(dynamic.field(
      "full_session_jsonl",
      dynamic.string,
    )(dyn))
    Ok(SessionHandoff(
