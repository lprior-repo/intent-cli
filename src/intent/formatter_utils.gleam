...
pub fn box_header(title: String) -> String {
  let top = emoji.box_tl <> string.repeat(emoji.box_h, box_width) <> emoji.box_tr
  let middle = center_in_box(title, box_width)
  let bottom = emoji.box_bl <> string.repeat(emoji.box_h, box_width) <> emoji.box_br
...
pub fn progress_bar_with_width(percentage: Float, width: Int) -> String {
  ...
  let filled_count =
    float.round(clamped /. 100.0 *. int.to_float(width))
...
pub fn section_header_with_count(icon: String, title: String, count: Int) -> String {
