...
fn center_in_box(text: String, width: Int) -> String {
  ...
  let padding = int.max(0, width - text_len)
...
pub fn progress_bar_with_width(percentage: Float, width: Int) -> String {
  ...
  let safe_width = int.clamp(width, 1, 100)
  let filled_count = float.round(clamped /. 100.0 *. int.to_float(safe_width))
  let empty_count = safe_width - filled_count
