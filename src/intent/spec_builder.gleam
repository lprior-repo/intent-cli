...
pub fn build_spec_from_session_with_analysis(
...
pub fn build_light_spec_from_session(session: InterviewSession) -> String {
  ...
  <> "package api\n\n"
  <> "name: \""
  <> escape_cue_string(name)
  <> "\"\n"
  <> "description: \""
  <> escape_cue_string(description)
  <> "\"\n\n"
  <> "behaviors: [\n"
  <> behaviors
  <> "]\n"
...
fn extract_light_behaviors_from_answers(answers: List(Answer)) -> String {
  ...
  |> list.index_map(fn(step, idx) {
    ...
    "\t{\n"
    <> "\t\tname: \""
    <> escape_cue_string(behavior_name)
    <> "\"\n"
    <> "\t\tintent: \""
    <> escape_cue_string(step)
    <> "\"\n"
    <> "\t\trequest: {\n"
    <> "\t\t\tmethod: \"GET\"\n"
    <> "\t\t\tpath: \"/\"\n"
...
fn build_spec_from_session_impl(
  session: InterviewSession,
  ...
  let spec = GeneratedCUE(package: "package api", imports: [], body: full_body)
