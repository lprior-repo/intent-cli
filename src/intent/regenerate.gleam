...
pub fn strategy_from_string(s: String) -> Result(RegenerationStrategy, String) {
  case string.lowercase(s) {
    ...
    "hybrid" -> Ok(Hybrid)
    _ -> Error("Unknown strategy: " <> s <> ". Valid: inversion, second_order, premortem, hybrid")
  }
}
...
pub fn apply_inversion(error: BeadError, bead_intent: String) -> RegenerationResult {
  let insights = [
    "Inverted expectation: Instead of success, what failures are possible?",
    "Error type '" <> error.error_type <> "' suggests checking: " <> inversion_checks(error.error_type),
  ]
...
pub fn apply_second_order(error: BeadError, dependencies: List(String)) -> RegenerationResult {
...
pub fn apply_hybrid(
  ...
  RegenerationResult(
    ...
    confidence: avg_confidence +. 0.1,  // Boost for comprehensive analysis
  )
}
