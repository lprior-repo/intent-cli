...
fn count_max_nesting_depth(json_str: String) -> Int {
  ...
  |> list.fold(#(0, 0), fn(acc, codepoint) {
    ...
    case string.utf_codepoint_to_int(codepoint) {
      123 | 91 -> #(current + 1, int.max(max, current + 1))
      // { or [
      125 | 93 -> #(int.max(current - 1, 0), max)
      // } or ]
      _ -> acc
    }
  })
...
/// Returns Json representation, using null for unrepresentable values
/// NOTE: Consider using explicit Result type to surface conversion failures
pub fn dynamic_to_json(data: Dynamic) -> Json {
  case dynamic.classify(data) {
    ...
    _unknown_type -> json.null()
  }
}

fn parse_response(data: Dynamic) -> Result(Response, List(DecodeError)) {
  use status <- result.try(dynamic.field("status", parse_status_code)(data))
...
/// Parse and validate HTTP status code (must be in range 100-599)
fn parse_status_code(data: Dynamic) -> Result(Int, List(DecodeError)) {
  use status <- result.try(dynamic.int(data))
  case status >= 100 && status <= 599 {
    True -> Ok(status)
    False -> {
      let error =
        dynamic.DecodeError(
          expected: "HTTP status code in range 100-599",
          found: "Integer(" <> int.to_string(status) <> ")",
          path: [],
        )
      Error([error])
    }
