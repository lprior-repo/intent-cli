...
pub type ValidationError {
  ValidationError(field: String, message: String, suggestion: String)
}
...
pub type LikelyCause {
  LikelyCause(cause: String, probability: Confidence, mitigation: String)
}
...
pub type PremortemData {
  PremortemData(assumed_failure: String, likely_causes: List(LikelyCause))
}
...
pub fn create_initial_directive(
  profile: String,
  _session_id: String,
) -> AIDirective {
...
pub fn process_answer(
  ...
  case validation_result {
    Error(errors) -> {
      ...
      AIDirective(..current, action: ClarifyAnswer, errors: Some(errors))
    }
    Ok(requirement) -> {
      ...
      let total =
        current.progress.questions_asked + current.progress.questions_remain
...
fn directive_to_cue_internal(
  ...
  <> "progress: {\n"
  <> "\tcurrent_round: "
  <> int.to_string(directive.progress.current_round)
  <> "\n"
  <> "\ttotal_rounds: "
  <> int.to_string(directive.progress.total_rounds)
  <> "\n"
  <> "\tquestions_asked: "
  <> int.to_string(directive.progress.questions_asked)
  <> "\n"
  <> "\tquestions_remain: "
  <> int.to_string(directive.progress.questions_remain)
  <> "\n"
  <> "\tpercent_complete: "
  <> float.to_string(directive.progress.percent_complete)
  <> "\n"
  <> "}\n"
  <> collected_str
  <> errors_str
...
pub fn empty_collected() -> CollectedRequirements {
  CollectedRequirements(
    ...
    unwanted: [],
    complex: [],
    inversions: InversionsData(security: [], usability: [], integration: []),
    premortem: PremortemData(assumed_failure: "", likely_causes: []),
  )
}
...
pub fn detect_ears_pattern(text: String) -> EARSPattern {
  ...
  case string.contains(lower, "if ") && string.contains(lower, "shall not") {
    True -> Unwanted
    False ->
      case string.contains(lower, "while ") && string.contains(lower, "when ") {
        True -> Complex
        False ->
          case string.contains(lower, "while ") {
            True -> StateDriven
            False ->
              case string.contains(lower, "when ") {
                True -> EventDriven
                False ->
                  case string.contains(lower, "where ") {
                    True -> Optional
                    False -> Ubiquitous
                  }
              }
...
pub fn validate_session_file(path: String) -> Result(Nil, String) {
  case
    string.starts_with(path, ".intent/session-")
    && string.ends_with(path, ".cue")
  {
    True -> Ok(Nil)
    False ->
      Error("Invalid session file path. Expected: .intent/session-{id}.cue")
  }
}
  ...
  case simplifile.verify_is_directory(".intent") {
    ...
    Error(_) -> {
      case simplifile.create_directory(".intent") {
        Ok(_) -> Ok(Nil)
        Error(e) ->
          Error("Failed to create .intent directory: " <> string.inspect(e))
      }
    }
  }
}
...
fn validate_answer(
  submission: AnswerSubmission,
) -> Result(EARSRequirement, List(ValidationError)) {
      ...
      case next_round <= 5 {
        True -> {
          let next_round_questions =
            interview_questions.get_questions_for_round(profile, next_round)
...
fn question_to_cue(q: Question) -> String {
  ...
  <> "\"\n"
  <> "\ttext: \""
  <> escape_cue_string(q.question)
  <> "\"\n"
  <> "\tcontext: \""
  <> escape_cue_string(q.context)
  <> "\"\n"
  <> "\texample: \""
  <> escape_cue_string(q.example)
  <> "\"\n"
  <> "\trequired: true\n"
  <> "}\n"
}
...
fn collected_to_cue_basic(collected: CollectedRequirements) -> String {
  "collected: {\n"
  <> "\tubiquitous: "
  <> requirements_list_to_cue(collected.ubiquitous)
  <> "\n"
  <> "\tevent_driven: "
  <> requirements_list_to_cue(collected.event_driven)
  <> "\n"
  <> "\tstate_driven: "
  <> requirements_list_to_cue(collected.state_driven)
  <> "\n"
  <> "\toptional: "
  <> requirements_list_to_cue(collected.optional)
  <> "\n"
  <> "\tunwanted: "
  <> requirements_list_to_cue(collected.unwanted)
  <> "\n"
  <> "\tcomplex: "
  <> requirements_list_to_cue(collected.complex)
  <> "\n"
  <> "}\n"
}
...
fn collected_to_cue_with_analysis(collected: CollectedRequirements) -> String {
  ...
  <> "\tinversions: {\n"
  <> "\t\tsecurity: "
  <> string_list_to_cue(collected.inversions.security)
  <> "\n"
  <> "\t\tusability: "
  <> string_list_to_cue(collected.inversions.usability)
  <> "\n"
  <> "\t\tintegration: "
  <> string_list_to_cue(collected.inversions.integration)
  <> "\n"
  <> "\t}\n"
  <> "\tpremortem: {\n"
  <> "\t\tassumed_failure: \""
  <> escape_cue_string(collected.premortem.assumed_failure)
  <> "\"\n"
  <> "\t\tlikely_causes: []\n"
  <> "\t}\n"
  <> "}\n"
    ...
    _ -> {
      let items =
        list.map(reqs, fn(r) {
          ...
          <> "\"\n"
          <> "\t\tpattern: \""
          <> pattern_to_string(r.pattern)
          <> "\"\n"
          <> "\t\tshall: \""
          <> escape_cue_string(r.shall)
          <> "\"\n"
          <> "\t\traw_text: \""
          <> escape_cue_string(r.raw_text)
          <> "\"\n"
          <> "\t}"
        })
      "[\n\t" <> string.join(items, ",\n\t") <> "\n]"
    ...
    _ -> {
      let items =
        list.map(errors, fn(e) {
          ...
          <> "\"\n"
          <> "\t\tmessage: \""
          <> escape_cue_string(e.message)
          <> "\"\n"
          <> "\t\tsuggestion: \""
          <> escape_cue_string(e.suggestion)
          <> "\"\n"
          <> "\t}"
        })
      "errors: [\n\t" <> string.join(items, ",\n\t") <> "\n]\n"
