  ...
  case maybe_answer {
    ...
    option.Some(answer_text) -> {
      ...
      case validate_answer(answer_text, question) {
        ...
        Error(msg) -> Error("Validation failed for " <> question.id <> ": " <> msg)
      }
    }
    ...
    option.None -> {
      case strict_mode {
        ...
        True -> {
          Error("Answer required for question " <> question.id <> " but not found in answers dictionary")
        }
        // Non-strict: fall back to prompting (would require I/O - skip for now)
        False -> {
          Error("Answer lookup failed for " <> question.id <> " and interactive mode not yet implemented")
        }
      }
    }
  }
