...
/// Ensure the .interview directory exists, creating it if needed
fn ensure_interview_directory() -> Result(Nil, String) {
  case simplifile.create_directory(".interview") {
    Ok(_) -> Ok(Nil)
    Error(simplifile.Eexist) -> Ok(Nil)
    // Directory already exists, that's fine
    Error(err) ->
      Error(format_file_error(err, "Failed to create .interview directory"))
  }
}
...
pub fn append_to_history(
  ...
  use _ <- result.try(ensure_interview_directory())
  ...
  |> result.map_error(fn(err) {
    format_file_error(err, "Failed to write history")
  })
}
...
pub fn append_session_to_jsonl(
  ...
  |> result.map_error(fn(err) {
    format_file_error(err, "Failed to write JSONL")
  })
}
...
pub fn list_sessions_from_jsonl(
  ...
  use content <- result.try(
    ...
    |> result.map_error(fn(err) {
      format_file_error(err, "Failed to read JSONL")
    }),
  )
...
fn conflict_decoder(
  ...
  use between <- result.try(case between_list {
    [first, second] -> Ok(#(first, second))
    _ -> Error([dynamic.DecodeError("between", "expected 2-element list", [])])
  })
  use description <- result.try(dynamic.field("description", dynamic.string)(
    json_value,
  ))
