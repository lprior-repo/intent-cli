...
pub fn mark_bead_blocked(
  ...
  let blocked_reason = option.Some(BlockedReason(
...
pub fn mark_bead_failed(
  ...
  let error = option.Some(BeadError(
...
fn feedback_to_cue(
  ...
  let error_str = case error {
    ...
    option.Some(err) -> {
      ...
      "\n\terror: {\n\t\ttype: \"" <> err.error_type <> "\"\n\t\tmessage: \"" <> escape_cue_string(err.message) <> "\"" <> trace_str <> "\n\t}"
    }
  }
  ...
  let blocked_str = case blocked_by {
    option.None -> ""
    option.Some(b) -> {
      "\n\tblocked_by: {\n\t\ttype: \"" <> b.blocker_type <> "\"\n\t\tdetails: \"" <> escape_cue_string(b.details) <> "\"\n\t\tunblocks_when: \"" <> escape_cue_string(b.unblocks_when) <> "\"\n\t}"
    }
  }

  "{\n\tbead_id: \"" <> bead_id <> "\"\n\tresult: \"" <> result_str <> "\"\n\treason: \"" <> escape_cue_string(reason) <> "\"\n\texecuted_at: \"" <> timestamp <> "\"\n\tduration_ms: " <> duration_str <> error_str <> blocked_str <> "\n}\n"
}
      ...
      case string.contains(trimmed, "-") {
        ...
        True -> {
          case string.split_once(trimmed, "-") {
            Error(Nil) -> False
            Ok(#(prefix, suffix)) -> {
              let prefix_ok = string.length(prefix) > 0 && string.length(prefix) <= 10
              let suffix_ok = string.length(suffix) == 3 && is_numeric_string(suffix)
              prefix_ok && suffix_ok
            }
          }
        }
...
fn parse_feedback_from_cue(_content: String) -> Result(List(BeadFeedback), FeedbackError) {
