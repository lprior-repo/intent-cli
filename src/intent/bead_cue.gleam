...
pub type Bead {
  Bead(
    id: String,           // Format: PREFIX-NNN (e.g., AUTH-001)
    title: String,
    what: String,         // Imperative, actionable
    why: String,          // Business value, user impact
    test_spec: TestSpec,  // Named test_spec to avoid reserved word
    done_when: List(String),
    file: String,         // Primary file to modify
    edge_cases: List(String),
    requires: List(String),  // Dependencies (bead IDs)
    effort: String,       // "5min" | "10min" | "15min" | "20min" | "30min"
...
pub type BeadFeedback {
  BeadFeedback(
    bead_id: String,
    result: String,    // "success" | "failed" | "blocked" | "skipped"
    reason: String,
    executed_at: String,
    duration_ms: Int,
    error_type: String,
...
pub fn bead_to_cue(bead: Bead) -> String {
  "{\n"
  <> "\tid: \"" <> bead.id <> "\"\n"
  <> "\ttitle: \"" <> escape_cue_string(bead.title) <> "\"\n"
  <> "\twhat: \"" <> escape_cue_string(bead.what) <> "\"\n"
  <> "\twhy: \"" <> escape_cue_string(bead.why) <> "\"\n"
  <> test_spec_to_cue(bead.test_spec)
  <> "\tdone_when: " <> string_list_to_cue(bead.done_when) <> "\n"
  <> "\tfile: \"" <> bead.file <> "\"\n"
  <> "\tedge_cases: " <> string_list_to_cue(bead.edge_cases) <> "\n"
  <> "\trequires: " <> string_list_to_cue(bead.requires) <> "\n"
  <> "\teffort: \"" <> bead.effort <> "\"\n"
  <> "\tstatus: \"" <> bead.status <> "\"\n"
  <> "}"
}
...
pub fn beads_to_cue(session_id: String, beads: List(Bead)) -> String {
  let beads_cue = list.map(beads, bead_to_cue)
    |> string.join(",\n")

  "// Beads for session: " <> session_id <> "\n"
...
fn test_spec_to_cue(spec: TestSpec) -> String {
  "\ttest: {\n"
  <> "\t\tcommand: \"" <> escape_cue_string(spec.command) <> "\"\n"
  <> "\t\texpect: {\n"
...
fn expectation_to_cue(expect: Expectation) -> String {
  ...
  let parts = case expect.exit_code != 0 || True {
    True -> list.append(parts, ["\t\t\texit_code: " <> int.to_string(expect.exit_code)])
    False -> parts
  }

  let parts = case string.length(expect.stdout_contains) > 0 {
    True -> list.append(parts, ["\t\t\tstdout_contains: \"" <> escape_cue_string(expect.stdout_contains) <> "\""])
    False -> parts
  }

  let parts = case string.length(expect.stdout_matches) > 0 {
    True -> list.append(parts, ["\t\t\tstdout_matches: \"" <> escape_cue_string(expect.stdout_matches) <> "\""])
    False -> parts
  }
  ...
  let parts = case expect.status > 0 {
    True -> list.append(parts, ["\t\t\tstatus: " <> int.to_string(expect.status)])
    False -> parts
  }

  let parts = case string.length(expect.body_contains) > 0 {
    True -> list.append(parts, ["\t\t\tbody_contains: \"" <> escape_cue_string(expect.body_contains) <> "\""])
    False -> parts
  }

  let parts = case string.length(expect.header_present) > 0 {
    True -> list.append(parts, ["\t\t\theader_present: \"" <> escape_cue_string(expect.header_present) <> "\""])
    False -> parts
  }
...
pub fn feedback_to_cue(feedback: BeadFeedback) -> String {
  ...
  <> "\treason: \"" <> escape_cue_string(feedback.reason) <> "\"\n"
  <> "\texecuted_at: \"" <> feedback.executed_at <> "\"\n"
  <> "\tduration_ms: " <> int.to_string(feedback.duration_ms) <> "\n"
...
fn error_section_to_cue(feedback: BeadFeedback) -> String {
  case string.length(feedback.error_type) > 0 {
    True -> {
      ...
      <> "\t\tmessage: \"" <> escape_cue_string(feedback.error_message) <> "\"\n"
      <> case string.length(feedback.error_trace) > 0 {
        True -> "\t\ttrace: \"" <> escape_cue_string(feedback.error_trace) <> "\"\n"
        False -> ""
      }
      <> "\t}\n"
    }
...
fn string_list_to_cue(items: List(String)) -> String {
  case items {
    [] -> "[]"
    _ -> {
      let quoted = list.map(items, fn(s) {
        "\"" <> escape_cue_string(s) <> "\""
      })
      "[" <> string.join(quoted, ", ") <> "]"
    }
