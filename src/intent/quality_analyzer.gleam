...
/// Gracefully handles partial specs by validating what exists
pub fn analyze_spec(spec: Spec) -> QualityReport {
  let behaviors =
    spec.features
    |> list.flat_map(fn(f) { f.behaviors })

  // Check for completely empty spec (graceful degradation)
  let has_data = case behaviors {
    [] -> False
    _ -> True
  }
  ...
  let overall_score = case has_data {
    True -> {
      let sum =
  ...
  let mut_issues = find_quality_issues(behaviors, spec.rules)

  // Add issue if spec is completely empty
  let mut_issues = case behaviors, list.is_empty(spec.features) {
    [], True -> [NoExamples, MissingExplanations, ..mut_issues]
    _, _ -> mut_issues
  }

  let suggestions = generate_suggestions(mut_issues, behaviors, spec.rules)

  // Add warning if spec appears incomplete
  let suggestions = case list.is_empty(spec.features) {
    True -> [
      "PARTIAL SPEC: No features defined. Add at least one feature with behaviors for meaningful analysis.",
      ..suggestions
    ]
    False ->
      case behaviors {
        [] -> [
          "PARTIAL SPEC: Features exist but have no behaviors. Add test behaviors to each feature.",
          ..suggestions
        ]
        _ -> suggestions
      }
