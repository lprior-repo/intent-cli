...
import intent/kirk/ears_parser.{
  type EarsPattern, type EarsRequirement,
...
fn calculate_clarity_score(req: EarsRequirement, raw_text: String) -> Int {
  ...
  int.min(100, base_score + pattern_bonus + length_bonus + behavior_bonus + vague_penalty)
}
...
pub fn analyze_bead_inversions(
  ...
  let covered_risks = count_covered_risks(edge_cases, security_risks, usability_risks, integration_risks)
  ...
  let suggested = generate_edge_case_suggestions(
    edge_cases,
    security_risks,
    usability_risks,
    integration_risks,
...
fn identify_security_risks(what: String) -> List(IdentifiedRisk) {
  ...
  // Token-related risks
  let risks = case
    string.contains(what, "jwt")
    || string.contains(what, "token")
  {
    True ->
      list.concat([
        risks,
...
pub fn calculate_quality_score(
  ...
  let weighted =
    int.to_float(completeness) *. 0.25
    +. int.to_float(testability) *. 0.40
    +. int.to_float(clarity) *. 0.35
