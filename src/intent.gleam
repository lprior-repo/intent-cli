...
pub fn main() {
  ...
  glint.run_and_handle(from: app, for: args, with: fn(_result) {
    // Commands handle their own exit codes via halt(), so this is just for
    // Glint-level errors (like invalid flags) that would otherwise exit 0
    halt(exit_error)
  })
}
        ...
        case loader.load_spec(spec_path) {
          Ok(spec) -> {
            case quality_analyzer.analyze_spec(spec) {
              ...
              Error(_e) -> {
                io.println_error("Error analyzing spec quality")
                halt(exit_error)
              }
            }
          }
...
@external(erlang, "intent_ffi", "mark_command_started")
fn mark_command_started() -> Nil
