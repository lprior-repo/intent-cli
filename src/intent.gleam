  ...
  glint.command(fn(input: glint.CommandInput) {
    ...
    case input.args {
      ...
      [] -> {
        ...
        io.println_error("")
        cli_ui.print_warning("IMPORTANT: Flags MUST use equals syntax:")
        io.println_error("  ✓ Correct:   --target=http://localhost:8080")
        io.println_error("  ✗ Incorrect: --target http://localhost:8080")
        io.println_error("")
        halt(exit_error)
      }
    }
...
fn run_check(
  ...
  case string.is_empty(target_url) {
    True -> {
      ...
      io.println_error("  intent check spec.cue --target=http://localhost:8080")
      io.println_error("")
      cli_ui.print_warning("Remember: Use --target=URL (with equals sign)")
      halt(exit_error)
    }
    False -> Nil
  }
      ...
      case dry_run {
        ...
        False -> {
          ...
          let confirmed = case auto_yes {
            True -> True
            False ->
              spec_preview.prompt_confirmation("Write spec to " <> path <> "?")
          }
