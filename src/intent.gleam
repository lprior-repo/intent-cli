...
pub fn main() {
  // Check for --no-color flag and set NO_COLOR environment variable
  let args = argv.load().arguments
  case list.any(args, fn(arg) { arg == "--no-color" }) {
    True -> {
  ...
  |> glint.add(at: ["about"], do: about_command())
  ...
  // Review commands
  |> glint.add(at: ["review-requirements"], do: review_requirements_command())
  |> glint.add(at: ["review-contracts"], do: review_contracts_command())
  |> glint.add(at: ["review-structure"], do: review_structure_command())
  |> glint.add(at: ["review-beads"], do: review_beads_command())
...
fn check_command() -> glint.Command(Nil) {
  glint.command(fn(input: glint.CommandInput) {
    // Load .intentrc configuration file
    let rc_config = config.load_intentrc()

    // Support configuration hierarchy: CLI flags > ENV vars > .intentrc
    let target_url = case flag.get_string(input.flags, "target") {
      Ok("") -> {
        let env_val = env_string("INTENT_TARGET", "")
        case env_val {
          "" -> config.get_string(rc_config, "target", "")
          _ -> env_val
        }
      }
      Ok(value) -> value
    ...
    let is_json = case flag.get_bool(input.flags, "json") {
      Ok(True) -> True
      _ -> {
        case env_bool("INTENT_JSON", False) {
          True -> True
          False -> config.get_bool(rc_config, "json", False)
        }
      }
    }

    let is_junit = case flag.get_bool(input.flags, "junit") {
      Ok(True) -> True
      _ -> False
    }

    let is_tap = case flag.get_bool(input.flags, "tap") {
      Ok(True) -> True
      _ -> False
    }

    let is_sarif = case flag.get_bool(input.flags, "sarif") {
      Ok(True) -> True
      _ -> False
    }

    let feature_filter = case flag.get_string(input.flags, "feature") {
      Ok("") -> {
        let env_val = env_string("INTENT_FEATURE", "")
        case env_val {
          "" -> config.get_string(rc_config, "feature", "")
          _ -> env_val
        }
      }
      Ok(value) -> value
    ...
    let only_filter = case flag.get_string(input.flags, "only") {
      Ok("") -> {
        let env_val = env_string("INTENT_ONLY", "")
        case env_val {
          "" -> config.get_string(rc_config, "only", "")
          _ -> env_val
        }
      }
      Ok(value) -> value
    ...
    let output_level = case flag.get_bool(input.flags, "verbose") {
      ...
      _ ->
        case flag.get_bool(input.flags, "quiet") {
          ...
          _ ->
            case env_bool("INTENT_VERBOSE", False) {
              True -> runner.Verbose
              False ->
                case env_bool("INTENT_QUIET", False) {
                  True -> runner.Quiet
                  False ->
                    case config.get_bool(rc_config, "verbose", False) {
                      True -> runner.Verbose
                      False ->
                        case config.get_bool(rc_config, "quiet", False) {
                          True -> runner.Quiet
                          False -> runner.Normal
                        }
                    }
    ...
    let dry_run = case flag.get_bool(input.flags, "dry-run") {
      Ok(True) -> True
      _ ->
        case env_bool("INTENT_DRY_RUN", False) {
          True -> True
          False -> config.get_bool(rc_config, "dry_run", False)
        }
    }
  ...
  |> glint.description(
    ...
    <> "  INTENT_DRY_RUN   - Override --dry-run (1/true/yes)",
  )
  |> glint.flag(
    "target",
    flag.string()
  ...
  |> glint.flag(
    ...
    flag.bool()
      |> flag.default(False)
      |> flag.description("Output results as JUnit XML for CI integration"),
  )
  |> glint.flag(
    "tap",
    flag.bool()
      |> flag.default(False)
      |> flag.description("Output results as TAP (Test Anything Protocol)"),
  )
  |> glint.flag(
    "sarif",
    flag.bool()
      |> flag.default(False)
      |> flag.description(
        "Output results as SARIF (Static Analysis Results Interchange Format)",
      ),
  )
  |> glint.flag(
    "feature",
  ...
  |> glint.flag(
    ...
    flag.bool()
      |> flag.default(False)
      |> flag.description(
        "Show what would be tested without executing requests",
      ),
  )
}
...
fn run_check(
  ...
  case loader.load_spec(spec_path) {
    ...
    Ok(spec) -> {
      ...
      // Build run options
      // Disable spinner in JSON/JUnit/TAP/SARIF mode or Quiet mode
      let show_spinner = case
        is_json,
        is_junit,
        is_tap,
        is_sarif,
      ...
      // Output results
      case is_json, is_junit, is_tap, is_sarif {
        True, _, _, _ -> {
          let json_result = output.spec_result_to_json(result)
          io.println(json.to_string(json_result))
        }
        _, True, _, _ -> {
          let junit_xml = output.spec_result_to_junit_xml(result, spec.name)
          io.println(junit_xml)
        }
        _, _, True, _ -> {
          let tap_output = output.spec_result_to_tap(result, spec.name)
          io.println(tap_output)
        }
        _, _, _, True -> {
          let sarif_output = output.spec_result_to_sarif(result, spec.name)
          io.println(sarif_output)
        }
...
/// Print dry run plan showing what would be tested
fn print_dry_run_plan(
  ...
  io.println("")
  cli_ui.print_info("DRY RUN MODE - No requests will be executed")
  io.println("")
  io.println("Target: " <> target_url)
  io.println("Spec: " <> spec.name <> " (v" <> spec.version <> ")")

  // Apply filters to get behaviors that would be tested
  let features_to_test = case options.feature_filter {
    Some(filter) -> list.filter(spec.features, fn(f) { f.name == filter })
    None -> spec.features
  }

  let behaviors_to_test =
    list.flat_map(features_to_test, fn(feature) {
      case options.behavior_filter {
        Some(filter) ->
          list.filter(feature.behaviors, fn(b) { b.name == filter })
          |> list.map(fn(b) { #(feature.name, b) })
        None -> list.map(feature.behaviors, fn(b) { #(feature.name, b) })
      }
    })

  let behavior_count = list.length(behaviors_to_test)
  let feature_count = list.length(features_to_test)
  ...
  list.each(features_to_test, fn(feature) {
    io.println("  " <> feature.name <> ":")
    list.each(feature.behaviors, fn(behavior) {
      io.println(
        "    - "
        <> behavior.name
        <> " ("
        <> types.method_to_string(behavior.request.method)
        <> " "
  ...
  io.println("")
  cli_ui.print_success("Dry run complete - spec is valid")
}
...
/// The `version` command - display version information
fn version_command() -> glint.Command(Nil) {
  glint.command(fn(_input: glint.CommandInput) {
    io.println("intent version 0.1.0")
    io.println("Contract-driven API testing tool")
    halt(exit_pass)
  })
  |> glint.description(
...
/// The `about` command - comprehensive explanation of Intent's purpose
fn about_command() -> glint.Command(Nil) {
  glint.command(fn(_input: glint.CommandInput) {
    io.println(
      "
┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
┃                                                                               ┃
┃  INTENT - Contract-Driven Requirements Engineering & API Testing             ┃
┃  ═══════════════════════════════════════════════════════════════════         ┃
┃                                                                               ┃
┃  🤖 AI-FIRST DESIGN: This tool is designed FOR AI AGENTS to execute          ┃
┃                      and USE TO RIGOROUSLY INTERVIEW HUMANS                  ┃
┃                                                                               ┃
┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
...
Intent transforms VAGUE human requirements into EXPLICIT, DETERMINISTIC work
items (called \"beads\") that AI agents can execute with ZERO ambiguity.

The tool enforces CONTRACT-DRIVEN DISCIPLINE through:

  • CUE schemas - Type-safe contracts that control all data flow
  • EARS patterns - Structured requirement syntax (THE SYSTEM SHALL...)
  • Mental lattices - Completeness frameworks that catch gaps
  • KIRK analysis - Quality scoring, inversion checking, coverage validation
  • Atomic beads - Single-responsibility work items with clear dependencies

═══════════════════════════════════════════════════════════════════════════════
THE CORE WORKFLOW
═══════════════════════════════════════════════════════════════════════════════

1. AI RUNS THE INTERVIEW COMMAND
   └─> intent interview --cue --profile api

2. AI USES INTENT TO GRILL THE HUMAN
   └─> Intent asks 25+ structured questions across 5 phases
   └─> Each answer must follow EARS patterns (THE SYSTEM SHALL...)
   └─> AI enforces rigor, rejects vague answers, demands specificity

3. INTENT GENERATES CUE SPECIFICATIONS
   └─> Human requirements → Validated CUE schemas
   └─> Mental lattice checks ensure completeness
   └─> Gap detection via inversion/second-order thinking

4. KIRK ANALYZES QUALITY
   └─> 5-dimension scoring (completeness, clarity, testability, etc.)
   └─> Inversion: \"What could fail? What's missing?\"
   └─> Coverage: HTTP methods, status codes, edge cases

5. INTENT GENERATES ATOMIC BEADS
   └─> Each bead = one unit of work with clear contract
   └─> Dependency tracking between beads
   └─> CUE-formatted for AI consumption
   └─> Executable with zero ambiguity

6. AI EXECUTES BEADS DETERMINISTICALLY
   └─> Beads integrate with 'bd' (beads issue tracker)
   └─> AI implements based on explicit contracts
   └─> Feedback loop refines failed/blocked beads
...
❌ WITHOUT INTENT:
   Human: \"Build a user API\"
   AI: \"Sure!\" → Guesses requirements → Builds wrong thing → Wasted time

✅ WITH INTENT:
   AI: \"Let me interview you systematically...\"
   → 25 structured questions
   → EARS-validated answers
   → CUE contracts generated
   → Quality scored (85/100 - needs auth improvements)
   → Gaps detected (missing error handling, no rate limiting)
   → Atomic beads created with dependencies
   → AI builds EXACTLY what was specified
...
• intent interview --cue --profile api
  └─> Start systematic requirements interview (AI-parseable JSON output)

• intent quality <spec.cue>
  └─> Score specification quality (completeness, clarity, testability, etc.)

• intent invert <spec.cue>
  └─> Inversion analysis: What could fail? What's missing?

• intent gaps <spec.cue>
  └─> Mental lattice gap detection using second-order thinking

• intent beads <session-id>
  └─> Generate atomic work items from validated requirements
...
═══════════════════════════════════════════════════════════════════════════════
  ...
  Human writes vague idea
      ↓
  AI runs Intent interview (grills human with discipline)
      ↓
  CUE schemas validate and structure everything
      ↓
  KIRK analyzes quality + detects gaps
      ↓
  Atomic beads generated with explicit contracts
      ↓
  AI executes deterministically (zero ambiguity)
      ↓
  Feedback loop refines failures
      ↓
  WORKING SOFTWARE that matches human's ACTUAL intent

CUE is the center: requirements, interview state, AI directives, beads, and
feedback all flow through validated CUE schemas.
...
AI agents should start here:
  1. intent about                    (you are here)
  2. intent interview --cue --help   (see full AI protocol)
  3. intent interview --cue --profile api  (start interviewing)

Full docs: https://github.com/yourusername/intent-cli
",
    )
    halt(exit_pass)
  })
  |> glint.description(
    "Comprehensive explanation of Intent's purpose and AI-first design\n\n"
    <> "Examples:\n"
    <> "  intent about",
  )
}
...
fn export_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    ...
    <> "  intent export spec.cue > output.json",
  )
}
...
fn interview_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    "🤖 AI-FIRST: Use this to RIGOROUSLY INTERVIEW humans and enforce requirements discipline\n\n"
    <> "═══════════════════════════════════════════════════════════════════════════════\n"
    <> "PURPOSE: Transform vague human ideas into EXPLICIT, CONTRACT-DRIVEN requirements\n"
    <> "═══════════════════════════════════════════════════════════════════════════════\n\n"
    <> "THIS COMMAND IS YOUR TOOL TO:\n"
    <> "  ✓ GRILL the human with 25+ structured questions across 5 phases\n"
    <> "  ✓ ENFORCE EARS patterns (THE SYSTEM SHALL... | WHEN... THE SYSTEM SHALL...)\n"
    <> "  ✓ REJECT vague answers and DEMAND specificity and completeness\n"
    <> "  ✓ CATCH gaps through mental lattice frameworks and inversion thinking\n"
    <> "  ✓ GENERATE validated CUE specifications with zero ambiguity\n"
    <> "  ✓ PRODUCE atomic beads that you can execute deterministically\n\n"
    <> "DO NOT accept vague requirements. DO NOT guess. USE THIS TOOL to force clarity.\n\n"
    <> "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
    ...
    <> "STEP 2: Parse CUE output, extract session.id and question.text\n"
    <> "   The output includes: question pattern, examples, hints for validation\n\n"
    <> "STEP 3: Present question to human, validate answer against EARS patterns\n"
    <> "   REJECT answers like: \"user authentication\" (too vague)\n"
    <> "   ACCEPT answers like: \"THE SYSTEM SHALL authenticate users via JWT tokens\"\n\n"
    <> "STEP 4: Submit validated answer (outputs next question OR completion):\n"
    <> "   intent interview --cue=true \\\n"
    <> "     --session=\"interview-XXXXX\" \\\n"
    <> "     --answer=\"THE SYSTEM SHALL authenticate users via JWT tokens\"\n\n"
    <> "STEP 5: REPEAT steps 2-4 until action=\"interview_complete\" (typically 25 questions)\n\n"
    <> "STEP 6: Generate beads from completed interview:\n"
    <> "   intent beads <session-id>\n\n"
    <> "Output format: CUE (structured, AI-parseable, type-safe)\n"
    ...
    <> "  • Unwanted: IF [condition] THEN THE SYSTEM SHALL [prevent/reject]\n\n"
    <> "CONTRACT-DRIVEN RIGOR:\n"
    <> "  • Every answer MUST follow EARS syntax (validated by patterns)\n"
    <> "  • Every requirement MUST be specific (no \"handle authentication\" - too vague)\n"
    <> "  • Every behavior MUST be testable (verifiable contracts, not aspirations)\n"
    <> "  • Mental lattices ensure completeness (discovers missing requirements)\n"
    <> "  • CUE schemas enforce type safety throughout the entire flow\n\n"
    <> "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
...
fn beads_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    "Generate atomic, executable work items (beads) from validated interview requirements\n\n"
    <> "🎯 PURPOSE: Transform CUE-validated requirements into DETERMINISTIC work items\n\n"
    <> "Each bead generated is:\n"
    <> "  • ATOMIC - Single responsibility, one clear task\n"
    <> "  • EXPLICIT - Zero ambiguity, contract-driven specification\n"
    <> "  • EXECUTABLE - AI can implement immediately without guessing\n"
    <> "  • DEPENDENCY-AWARE - Tracks what must be done first\n"
    <> "  • CUE-VALIDATED - Type-safe contracts, not aspirational todos\n\n"
    <> "After interview completes, use this command to generate beads:\n"
    <> "  1. Run interview: intent interview --cue --profile api\n"
    <> "  2. Complete all questions until action=\"interview_complete\"\n"
    <> "  3. Generate beads: intent beads <session-id>\n"
    <> "  4. Beads exported to .beads/issues.jsonl for bd (beads tracker)\n"
    <> "  5. AI can now claim and execute beads deterministically\n\n"
    <> "Examples:\n"
...
fn execute_beads_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    ...
    <> "Examples:\n"
    <> "  intent execute-beads interview-abc123def456",
  )
}
...
fn plan_approve_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    ...
    <> "  intent plan-approve abc123 --notes 'approved for release'",
  )
  |> glint.flag(
    "yes",
    flag.bool()
...
fn kirk_quality_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    "KIRK: Multi-dimensional quality scoring with contract-driven rigor\n\n"
    <> "🔍 PURPOSE: Objectively score specification quality before implementation\n\n"
    <> "Analyzes 5 critical dimensions:\n"
    <> "  1. COMPLETENESS (0-100) - Are all behaviors, checks, rules defined?\n"
    <> "  2. CONSISTENCY (0-100) - Do patterns align? Any contradictions?\n"
    <> "  3. TESTABILITY (0-100) - Can every behavior be verified automatically?\n"
    <> "  4. CLARITY (0-100) - Are requirements explicit and unambiguous?\n"
    <> "  5. SECURITY (0-100) - Auth, validation, error handling covered?\n\n"
    <> "OVERALL SCORE: Weighted average across all dimensions\n\n"
    <> "Use this to:\n"
    <> "  • Validate requirements before starting implementation\n"
    <> "  • Identify weak areas needing refinement (< 70 = needs work)\n"
    <> "  • Enforce quality gates (e.g., require 85+ before generating beads)\n"
    <> "  • Track improvement over interview iterations\n\n"
    <> "Examples:\n"
    <> "  intent quality spec.cue\n"
...
fn kirk_invert_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    "KIRK: Inversion thinking - systematically discover what could fail\n\n"
    <> "⚡ PURPOSE: Challenge requirements by asking \"What could go wrong?\"\n\n"
    <> "Applies second-order thinking to detect missing failure modes:\n"
    <> "  • SECURITY GAPS - Authentication bypass? Injection vectors? Auth escalation?\n"
    <> "  • USABILITY GAPS - What if user retries? Cancels? Provides invalid input?\n"
    <> "  • INTEGRATION GAPS - External API failures? Network timeouts? Rate limits?\n"
    <> "  • EDGE CASES - Empty lists? Null values? Concurrent operations?\n\n"
    <> "For each gap, suggests missing behaviors you should add:\n"
    <> "  Example: \"Handle 429 rate limit response from authentication service\"\n\n"
    <> "INVERSION SCORE (0-100):\n"
    <> "  100 = Comprehensive failure handling\n"
    <> "  < 70 = Critical gaps exist, DO NOT implement yet\n\n"
    <> "Use this to:\n"
    <> "  • Catch failure modes humans didn't think to specify\n"
    <> "  • Prevent production incidents from missing error handling\n"
    <> "  • Force completeness before implementation begins\n\n"
    <> "Examples:\n"
    <> "  intent invert spec.cue\n"
...
fn kirk_gaps_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    "KIRK: Mental lattice gap detection using systematic thinking frameworks\n\n"
    <> "🧠 PURPOSE: Apply multiple mental models to discover hidden requirement gaps\n\n"
    <> "Combines 5 complementary detection strategies:\n\n"
    <> "1. INVERSION GAPS - \"What could fail?\" (auth bypass, timeout, retry logic)\n"
    <> "2. SECOND-ORDER GAPS - \"Then what?\" (cascading effects, side effects)\n"
    <> "3. CHECKLIST GAPS - Mental lattices (CRUD, HTTP methods, auth flow)\n"
    <> "4. COVERAGE GAPS - Missing HTTP status codes, methods not tested\n"
    <> "5. SECURITY GAPS - OWASP Top 10, injection, auth, validation\n\n"
    <> "Each gap includes:\n"
    <> "  • SEVERITY: critical | high | medium | low\n"
    <> "  • CATEGORY: What type of gap (auth, validation, error handling)\n"
    <> "  • DESCRIPTION: Specific missing requirement\n"
    <> "  • SUGGESTED FIX: Concrete behavior to add\n\n"
    <> "Example output:\n"
    <> "  [CRITICAL] Missing password reset rate limiting\n"
    <> "    → Add: THE SYSTEM SHALL rate limit password reset to 3 attempts per hour\n\n"
    <> "Use this to:\n"
    <> "  • Systematically discover what the human forgot to specify\n"
    <> "  • Validate completeness before implementation\n"
    <> "  • Prevent \"I didn't think of that\" production bugs\n"
    <> "  • Enforce rigorous requirements discipline\n\n"
    <> "Examples:\n"
    <> "  intent gaps spec.cue\n"
...
fn kirk_compact_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    ...
    <> "  intent compact spec.cue --tokens",
  )
  |> glint.flag(
    "tokens",
    flag.bool()
...
fn ears_interview_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    ...
    <> "  intent ears-interview --profile Cli --name MyTool",
  )
}
...
fn generate_contract_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    ...
    <> "  intent generate-contract requirements.md --name UserAPI",
  )
  |> glint.flag(
    "json",
    flag.bool()
...
fn plan_structure_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    ...
    <> "  intent plan-structure --project MyProject --json",
  )
  |> glint.flag(
    "project",
    flag.string()
...
fn generate_beads_command() -> glint.Command(Nil) {
  ...
  |> glint.description(
    ...
    <> "  intent generate-beads --project MyProject --json",
  )
  |> glint.flag(
    "project",
    flag.string()
...
/// The `review-requirements` command - review EARS requirements
fn review_requirements_command() -> glint.Command(Nil) {
  glint.command(fn(input: glint.CommandInput) {
    let save_checkpoint =
      flag.get_bool(input.flags, "checkpoint")
      |> result.unwrap(False)
    ...
    case input.args {
        ...
        case simplifile.read(requirements_file) {
          Ok(content) -> {
            ...
            case parse_result.requirements {
              ...
              requirements -> {
                // Display review
                let review = review_gates.review_requirements(requirements)
                io.println(review)
                ...
                case save_checkpoint {
                  True -> {
                    case
                      review_gates.save_requirements_checkpoint(
                        requirements,
                        "Review checkpoint",
                      )
                    {
                      Ok(_) -> {
                        io.println(
                          "✓ Checkpoint saved to .intent/checkpoints/requirements.json",
                        )
                      }
                      Error(err) -> {
                        cli_ui.print_error(
                          "Failed to save checkpoint: "
                          <> review_gates.describe_error(err),
                        )
                      }
                    }
                  }
      ...
      [] -> {
        cli_ui.print_error(
          "Usage: intent review-requirements <requirements-file>",
        )
        io.println("")
        io.println("Review EARS requirements with validation questions:")
        io.println("  • Pattern appropriateness")
        io.println("  • Clarity and specificity")
        io.println("  • Completeness")
        io.println("  • Avoidance of vague terms")
        io.println("")
        io.println("Options:")
        io.println("  --checkpoint    Save checkpoint to .intent/checkpoints/")
        halt(exit_error)
      }
    }
  })
  |> glint.description(
    ...
    <> "Examples:\n"
    <> "  intent review-requirements requirements.md",
  )
  |> glint.flag(
    "checkpoint",
    flag.bool()
      |> flag.default(False)
      |> flag.description("Save checkpoint"),
  )
}

/// The `review-contracts` command - review KIRK contracts
fn review_contracts_command() -> glint.Command(Nil) {
  glint.command(fn(input: glint.CommandInput) {
    let save_checkpoint =
      flag.get_bool(input.flags, "checkpoint")
      |> result.unwrap(False)
    ...
    case input.args {
            ...
            case parse_result.requirements {
              ...
              requirements -> {
                ...
                case kirk_contract.generate_contracts(requirements) {
                  Ok(contracts) -> {
                    // Display review
                    let review = review_gates.review_contracts(contracts)
                    io.println(review)
                    ...
                    case save_checkpoint {
                      True -> {
                        case
                          review_gates.save_contracts_checkpoint(
                            contracts,
                            "Review checkpoint",
                          )
                        {
                          Ok(_) -> {
                            io.println(
                              "✓ Checkpoint saved to .intent/checkpoints/contracts.json",
                            )
                          }
                          Error(err) -> {
                            cli_ui.print_error(
      ...
      [] -> {
        cli_ui.print_error("Usage: intent review-contracts <requirements-file>")
        io.println("")
        io.println("Review KIRK contracts with validation questions:")
        io.println("  • Precondition completeness")
        io.println("  • Postcondition testability")
        io.println("  • Second-order effect identification")
        io.println("  • Contract confidence")
        io.println("")
        io.println("Options:")
        io.println("  --checkpoint    Save checkpoint to .intent/checkpoints/")
        halt(exit_error)
  ...
  |> glint.description(
    ...
    <> "Examples:\n"
    <> "  intent review-contracts --project MyProject",
  )
  |> glint.flag(
    "checkpoint",
    flag.bool()
...
/// The `review-structure` command - review project structure
fn review_structure_command() -> glint.Command(Nil) {
  glint.command(fn(input: glint.CommandInput) {
    let save_checkpoint =
      flag.get_bool(input.flags, "checkpoint")
      |> result.unwrap(False)
    ...
    case input.args {
        ...
        case simplifile.read(requirements_file) {
          Ok(content) -> {
            ...
            case parse_result.requirements {
              ...
              requirements -> {
                // Generate contracts and structure
                case kirk_contract.generate_contracts(requirements) {
                  Ok(contracts) -> {
                    case
                      structure_planner.plan_structure(project_name, contracts)
                    {
                      Ok(structure) -> {
                        // Display review
                        let review = review_gates.review_structure(structure)
                        io.println(review)
                        ...
                        case save_checkpoint {
                          True -> {
                            case
                              review_gates.save_structure_checkpoint(
                                structure,
                                "Review checkpoint",
                              )
                            {
                              Ok(_) -> {
                                io.println(
                                  "✓ Checkpoint saved to .intent/checkpoints/structure.json",
                                )
                              }
                              Error(err) -> {
                                cli_ui.print_error(
      ...
      [] -> {
        cli_ui.print_error("Usage: intent review-structure <requirements-file>")
        io.println("")
        io.println("Review project structure with validation questions:")
        io.println("  • Epic/feature/task hierarchy logic")
        io.println("  • Task sizing appropriateness")
        io.println("  • Wave-based dependency order")
        io.println("  • Parallelism score acceptability")
        io.println("")
        io.println("Options:")
        io.println("  --project=NAME    Project name (default: 'Project')")
        io.println(
          "  --checkpoint      Save checkpoint to .intent/checkpoints/",
        )
        halt(exit_error)
      }
    }
  ...
  |> glint.description(
    ...
    <> "Examples:\n"
    <> "  intent review-structure --project MyProject",
  )
  |> glint.flag(
    "project",
    flag.string()
...
/// The `review-beads` command - review generated beads (checkpoint only)
fn review_beads_command() -> glint.Command(Nil) {
  glint.command(fn(_input: glint.CommandInput) {
    cli_ui.print_error("This command reviews beads after generation")
    io.println("")
    io.println("To review beads:")
    io.println("  1. Run: intent generate-beads <requirements-file>")
    io.println("  2. Review in bd: bd list --status=open")
    io.println("  3. Check details: bd show <id>")
    io.println("  4. View dependencies: bd show <id>")
    io.println("")
    io.println("Or use bd commands directly:")
    io.println("  bd ready          Show available work")
    io.println("  bd blocked        Show blocked issues")
    io.println("  bd stats          Project statistics")
    halt(exit_pass)
  })
  |> glint.description(
    "Review generated beads (guidance command)\n\n"
    <> "Examples:\n"
    <> "  intent review-beads --project MyProject",
  )
}
