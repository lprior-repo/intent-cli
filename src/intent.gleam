...
fn run_check(
  ...
  case loader.load_spec(spec_path) {
    ...
    Ok(spec) -> {
      ...
      // Build run options
      // Disable spinner in JSON/JUnit/TAP mode or Quiet mode
      let show_spinner = case is_json, is_junit, is_tap, output_level {
        True, _, _, _ -> False
        _, True, _, _ -> False
        _, _, True, _ -> False
        _, _, _, runner.Quiet -> False
        _, _, _, _ -> True
      }
