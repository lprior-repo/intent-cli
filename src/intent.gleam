        ...
        let response =
          json_output.failure(
            ...
            json.object([
              #(
                "usage",
                json.string("intent feedback --results <check-output.json> [--json]"),
              ),
              #(
                "description",
      ...
      [] -> {
        ...
        case interview_storage.list_all_history(history_path) {
          ...
          Ok(all_snapshots) -> {
            ...
            let data =
              json.object([
                #("snapshots", json.array(snapshots, interview_storage.snapshot_to_json)),
      ...
      _ -> {
        ...
        let response =
          json_output.failure(
            ...
            "diff",
            json.object([
              #("usage", json.string("intent diff <spec1.cue> <spec2.cue> [--json]")),
              #(
                "description",
                json.string(
                  "Compare two spec versions and show differences",
                ),
              ),
            ]),
            [error],
        ...
        let response =
          json_output.failure(
            ...
            json.object([
              #(
                "usage",
                json.string("intent ears <requirements.md> [--output text|cue|json] [--out <file>]"),
              ),
              #(
                "description",
                json.string("EARS requirements parser - converts natural language to CUE behaviors"),
              ),
              #(
                "patterns",
                json.object([
                  #("ubiquitous", json.string("THE SYSTEM SHALL [behavior]")),
                  #("event_driven", json.string("WHEN [trigger] THE SYSTEM SHALL [behavior]")),
                  #("state_driven", json.string("WHILE [state] THE SYSTEM SHALL [behavior]")),
                  #("optional", json.string("WHERE [condition] THE SYSTEM SHALL [behavior]")),
                  #("unwanted", json.string("IF [condition] THEN THE SYSTEM SHALL NOT")),
                ]),
              ),
            ]),
            [error],
