...
-opaque subject(FRA) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, FRA}.

-type do_not_leak() :: any().

-type selector(FRB) :: any() | {gleam_phantom, FRB}.
...
-type call_error(FRC) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, FRC}.
...
-file("src/gleam/erlang/process.gleam", 131).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 189).
?DOC(
...
-spec select(selector(FRV), integer()) -> {ok, FRV} | {error, nil}.
select(From, Within) ->
    gleam_erlang_ffi:select(From, Within).

-file("src/gleam/erlang/process.gleam", 198).
?DOC(
...
-spec select_forever(selector(FRZ)) -> FRZ.
select_forever(From) ->
    gleam_erlang_ffi:select(From).

-file("src/gleam/erlang/process.gleam", 207).
?DOC(
...
-spec map_selector(selector(FSB), fun((FSB) -> FSD)) -> selector(FSD).
map_selector(A, B) ->
    gleam_erlang_ffi:map_selector(A, B).

-file("src/gleam/erlang/process.gleam", 216).
?DOC(
...
-spec merge_selector(selector(FSF), selector(FSF)) -> selector(FSF).
merge_selector(A, B) ->
    gleam_erlang_ffi:merge_selector(A, B).

-file("src/gleam/erlang/process.gleam", 260).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 232).
?DOC(
...
-spec selecting_trapped_exits(selector(FSJ), fun((exit_message()) -> FSJ)) -> selector(FSJ).
...
-file("src/gleam/erlang/process.gleam", 273).
?DOC(
...
-spec selecting(selector(FSM), subject(FSO), fun((FSO) -> FSM)) -> selector(FSM).
...
-file("src/gleam/erlang/process.gleam", 299).
?DOC(
...
-spec selecting_record3(
    selector(FTA),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FTA)
) -> selector(FTA).
...
-file("src/gleam/erlang/process.gleam", 333).
?DOC(
...
-spec selecting_record4(
    selector(FTE),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FTE)
) -> selector(FTE).
...
-file("src/gleam/erlang/process.gleam", 351).
?DOC(
...
-spec selecting_record5(
    selector(FTI),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FTI)
) -> selector(FTI).
...
-file("src/gleam/erlang/process.gleam", 369).
?DOC(
...
-spec selecting_record6(
    selector(FTM),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FTM)
) -> selector(FTM).
...
-file("src/gleam/erlang/process.gleam", 387).
?DOC(
...
-spec selecting_record7(
    selector(FTQ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FTQ)
) -> selector(FTQ).
...
-file("src/gleam/erlang/process.gleam", 408).
?DOC(
...
-spec selecting_record8(
    selector(FTU),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> FTU)
) -> selector(FTU).
...
-file("src/gleam/erlang/process.gleam", 458).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 482).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 489).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 529).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 574).
?DOC(
...
-spec deselecting_process_down(selector(FUN), process_monitor()) -> selector(FUN).
...
-file("src/gleam/erlang/process.gleam", 589).
?DOC(
...
-spec try_call(subject(FUQ), fun((subject(FUS)) -> FUQ), integer()) -> {ok, FUS} |
    {error, call_error(FUS)}.
...
-file("src/gleam/erlang/process.gleam", 630).
?DOC(
    " Send a message to a process and wait for a reply.\n"
    "\n"
    " If the receiving process exits or does not reply within the allowed amount\n"
...
-spec call(subject(FUX), fun((subject(FUZ)) -> FUX), integer()) -> FUZ.
...
-file("src/gleam/erlang/process.gleam", 659).
?DOC(
...
-spec try_call_forever(subject(FVF), fun((subject(FVH)) -> FVF)) -> {ok, FVH} |
...
-file("src/gleam/erlang/process.gleam", 646).
?DOC(
...
-spec call_forever(subject(FVB), fun((subject(FVD)) -> FVB)) -> FVD.
...
-file("src/gleam/erlang/process.gleam", 697).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 704).
...
-file("src/gleam/erlang/process.gleam", 716).
?DOC(" Send a message over a channel after a specified number of milliseconds.\n").
-spec send_after(subject(FVO), integer(), FVO) -> timer().
...
-file("src/gleam/erlang/process.gleam", 764).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 780).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 831).
?DOC(
...
-file("src/gleam/erlang/process.gleam", 854).
?DOC(
