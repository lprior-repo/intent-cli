...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, GAW} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, GAW} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, GAS} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, GAS} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, GAO} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, GAO} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec nullable(gleam@option:option(GBC), fun((GBC) -> json())) -> json().
...
-spec array(list(GBG), fun((GBG) -> json())) -> json().
