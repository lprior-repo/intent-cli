...
-spec map_first({YJ, YK}, fun((YJ) -> YL)) -> {YL, YK}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({YM, YN}, fun((YN) -> YO)) -> {YM, YO}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
