...
-opaque set(EZK) :: {set, gleam@dict:dict(EZK, list(nil))}.
...
-spec contains(set(EZV), EZV) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(EZX), EZX) -> set(EZX).
delete(Set, Member) ->
...
-spec fold(set(FAG), FAI, fun((FAI, FAG) -> FAI)) -> FAI.
...
-spec filter(set(FAJ), fun((FAJ) -> boolean())) -> set(FAJ).
...
-spec drop(set(FAQ), list(FAQ)) -> set(FAQ).
drop(Set, Disallowed) ->
...
-spec take(set(FAU), list(FAU)) -> set(FAU).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 267).
-spec order(set(FAY), set(FAY)) -> {set(FAY), set(FAY)}.
order(First, Second) ->
...
-spec intersection(set(FBH), set(FBH)) -> set(FBH).
...
-spec difference(set(FBL), set(FBL)) -> set(FBL).
...
-spec is_subset(set(FBP), set(FBP)) -> boolean().
...
-spec is_disjoint(set(FBS), set(FBS)) -> boolean().
...
-spec insert(set(EZS), EZS) -> set(EZS).
insert(Set, Member) ->
...
-spec from_list(list(FAD)) -> set(FAD).
from_list(Members) ->
...
-spec map(set(FAM), fun((FAM) -> FAO)) -> set(FAO).
map(Set, Fun) ->
...
-spec union(set(FBD), set(FBD)) -> set(FBD).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(FBV), set(FBV)) -> set(FBV).
