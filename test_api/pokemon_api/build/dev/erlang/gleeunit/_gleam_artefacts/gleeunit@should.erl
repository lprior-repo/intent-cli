...
-spec equal(GKN, GKN) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(GKO, GKO) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, GKP} | {error, any()}) -> GKP.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, GKU}) -> GKU.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(GKX)) -> GKX.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
