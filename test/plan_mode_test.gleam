...
import intent/plan_mode.{
  type BeadStatus, type Effort, type ExecutionPhase, type ExecutionPlan,
  type PlanBead, type PlanError, Blocked, CyclicDependency, Effort10min,
  Effort15min, Effort20min, Effort30min, Effort5min, ExecutionPhase,
  ExecutionPlan, Failed, InProgress, MissingDependency, Pending, PlanBead,
  SessionNotFound,
}
...
pub fn format_plan_human_with_blockers_test() {
  let plan =
    ExecutionPlan(
      ...
      total_effort: "20min",
      risk: plan_mode.High,
      blockers: ["bead-X: Blocked by external dependency", "bead-Y: Failed test"],
    )
