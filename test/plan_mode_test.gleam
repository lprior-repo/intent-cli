...
// =============================================================================
// TEST: bead_waves - Topological sort returning parallel execution waves
// =============================================================================

pub fn bead_waves_empty_list_test() {
  // Empty list should return empty waves
  let result = plan_mode.bead_waves([])

  should.be_ok(result)

  let waves = case result {
    Ok(w) -> w
    Error(_) -> panic as "Expected Ok"
  }
...
pub fn bead_waves_single_bead_no_deps_test() {
  // Single bead with no dependencies -> one wave with one bead
  let beads = [
    PlanBead(
      id: "bead-1",
      title: "First bead",
  ...
  let result = plan_mode.bead_waves(beads)
  ...
  should.equal(list.length(waves), 1)

  let first_wave = case waves {
    [w, ..] -> w
    [] -> panic as "Expected at least one wave"
  }

  should.equal(first_wave, ["bead-1"])
}

pub fn bead_waves_two_independent_beads_test() {
  // Two beads with no dependencies -> one wave with both beads
  let beads = [
    PlanBead(
      id: "bead-a",
      title: "Bead A",
  ...
  // Both beads should be in first wave (order doesn't matter, just check count)
  should.equal(list.length(first_wave), 2)
  should.be_true(list.contains(first_wave, "bead-a"))
  should.be_true(list.contains(first_wave, "bead-b"))
}

pub fn bead_waves_linear_chain_test() {
  // Chain: A -> B -> C (three waves, one bead each)
  let beads = [
    PlanBead(
      id: "bead-a",
      title: "Bead A",
  ...
  should.equal(list.length(waves), 3)

  // Check wave contents in order
  case waves {
    [wave1, wave2, wave3] -> {
      should.equal(wave1, ["bead-a"])
      should.equal(wave2, ["bead-b"])
      should.equal(wave3, ["bead-c"])
    }
    _ -> panic as "Expected exactly 3 waves"
  }
}

pub fn bead_waves_parallel_then_sequential_test() {
  // A, B parallel (depth 0) -> C depends on A -> D depends on C (depth 2)
  // Waves: [A, B], [C], [D]
  let beads = [
    PlanBead(
      id: "bead-a",
  ...
  case waves {
    [wave1, wave2, wave3] -> {
      // Wave 1: A and B (order doesn't matter)
      should.equal(list.length(wave1), 2)
      should.be_true(list.contains(wave1, "bead-a"))
      should.be_true(list.contains(wave1, "bead-b"))
      // Wave 2: C
      should.equal(wave2, ["bead-c"])
      // Wave 3: D
      should.equal(wave3, ["bead-d"])
    }
    _ -> panic as "Expected exactly 3 waves"
  }
}

pub fn bead_waves_diamond_dependency_test() {
  // Diamond: A, B both depend on C; D depends on both A and B
  // Waves: [C], [A, B], [D]
  let beads = [
    PlanBead(
      id: "bead-c",
  ...
  case waves {
    [wave1, wave2, wave3] -> {
      // Wave 1: C
      should.equal(wave1, ["bead-c"])
      // Wave 2: A and B (order doesn't matter)
      should.equal(list.length(wave2), 2)
      should.be_true(list.contains(wave2, "bead-a"))
      should.be_true(list.contains(wave2, "bead-b"))
      // Wave 3: D
      should.equal(wave3, ["bead-d"])
    }
    _ -> panic as "Expected exactly 3 waves"
...
pub fn bead_waves_complex_parallel_structure_test() {
  // Complex: A, B (depth 0); C depends on A; D depends on B; E depends on C,D
  // Waves: [A, B], [C, D], [E]
  let beads = [
    PlanBead(
      id: "bead-a",
  ...
  case waves {
    [wave1, wave2, wave3] -> {
      ...
      // Wave 2: C and D
      should.equal(list.length(wave2), 2)
      should.be_true(list.contains(wave2, "bead-c"))
      should.be_true(list.contains(wave2, "bead-d"))
      // Wave 3: E
      should.equal(wave3, ["bead-e"])
    }
    _ -> panic as "Expected exactly 3 waves"
  }
}

pub fn bead_waves_missing_dependency_error_test() {
  // Bead with missing dependency should error
  let beads = [
    PlanBead(
      id: "bead-1",
...
pub fn bead_waves_cyclic_dependency_error_test() {
  ...
  case result {
    Error(plan_mode.CyclicDependency(_)) -> True
    _ -> panic as "Expected CyclicDependency error"
  }
  |> should.be_true
}

pub fn bead_waves_self_dependency_error_test() {
  // Bead depends on itself
  let beads = [
    PlanBead(
      id: "bead-a",
      title: "Bead A",
...
pub fn bead_waves_three_bead_cycle_error_test() {
  // A -> B -> C -> A (cycle involving three beads)
  let beads = [
    PlanBead(
      id: "bead-a",
      title: "Bead A",
...
pub fn bead_waves_multiple_missing_deps_test() {
  // Bead with multiple missing dependencies should error on first
  let beads = [
    PlanBead(
      id: "bead-1",
      title: "Bead 1",
  ...
  case result {
    Error(plan_mode.MissingDependency(bead, _)) -> {
      should.equal(bead, "bead-1")
    }
    _ -> panic as "Expected MissingDependency error"
  }
