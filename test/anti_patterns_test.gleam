...
pub fn check_anti_patterns_no_match_test() {
  ...
  let pattern = make_anti_pattern("exposed-password", "Passwords in response", bad, good)
...
pub fn check_anti_patterns_multiple_patterns_test() {
  ...
  let pattern1 = make_anti_pattern("exposed-password", "Password leak", bad1, good1)

  // Pattern 2: internal ID exposure
  let bad2 = json.object([#("internal_id", json.int(123))])
  let good2 = json.object([#("id", json.int(123))])
  let pattern2 = make_anti_pattern("internal-id", "Internal IDs exposed", bad2, good2)
  ...
  let results = check_anti_patterns([pattern1, pattern2], response, "test_behavior")
...
pub fn check_anti_patterns_nested_fields_test() {
  ...
  let pattern = make_anti_pattern("nested-password", "Nested password leak", bad, good)
...
pub fn check_anti_patterns_deeply_nested_test() {
  ...
  let pattern = make_anti_pattern("secret-key-leak", "Secret key exposed", bad, good)
...
pub fn check_anti_patterns_empty_response_test() {
  ...
  let pattern = make_anti_pattern("internal-error", "Internal errors exposed", bad, good)
...
pub fn check_anti_patterns_same_keys_different_values_test() {
  ...
  let pattern = make_anti_pattern("verbose-error", "Verbose error codes", bad, good)
...
pub fn format_anti_pattern_detected_test() {
  ...
  let result = AntiPatternDetected(
...
pub fn check_anti_patterns_identical_examples_test() {
  ...
  let example = json.object([#("id", json.int(1)), #("name", json.string("test"))])
