...
import intent/bead_validator.{
  type BeadKirkMetadata, BeadContract, BeadEARS, BeadInversion, BeadKirkMetadata,
  BeadQuality, SevError, ValidationIssue, analyze_bead_inversions,
  calculate_quality_score, validate_bead_ears, validate_bead_kirk,
}
...
fn make_low_quality_metadata() -> BeadKirkMetadata {
  BeadKirkMetadata(
    ...
    contract: Some(
      BeadContract(preconditions: [], postconditions: [], invariants: []),
    ),
    inversion: Some(
      BeadInversion(
        security_risks: [],
...
fn make_minimal_metadata() -> BeadKirkMetadata {
  BeadKirkMetadata(ears: None, contract: None, inversion: None, quality: None)
}
...
pub fn validate_bead_ears_event_driven_pattern_test() {
  ...
  let what =
    "WHEN the user submits the registration form THE SYSTEM SHALL create a new account"
...
pub fn validate_bead_ears_optional_pattern_test() {
  ...
  let what =
    "WHERE the user has admin privileges THE SYSTEM SHALL show the admin panel"
...
pub fn validate_bead_ears_unwanted_pattern_test() {
  ...
  let what =
    "IF the account is suspended THE SYSTEM SHALL NOT allow access to resources"
...
pub fn validate_bead_ears_complex_pattern_test() {
  ...
  let what =
    "WHILE the user is active WHEN the session expires THE SYSTEM SHALL automatically refresh the authentication token"
...
pub fn calculate_quality_score_testability_weighted_test() {
  ...
  let high_test = calculate_quality_score(80, 100, 80)
  // testability 100
  let low_test = calculate_quality_score(80, 60, 80)
  // testability 60
