...
pub fn checker_jwt_not_string_test() {
  ...
  let body = json.object([#("token", json.int(12_345))])
  let actual = make_execution_result(body)
  let expected = make_expected_response([#("token", Check("jwt", "JWT check"))])
  let ctx = empty_context()
...
pub fn checker_regex_simple_match_test() {
  ...
  let expected =
    make_expected_response([
      #(
        "code",
        Check("string matching ^[A-Z0-9]+$", "Must be alphanumeric uppercase"),
      ),
    ])
  let ctx = empty_context()
...
pub fn checker_regex_email_pattern_test() {
  ...
  let expected =
    make_expected_response([
      #(
        "email",
        Check("string matching ^[a-z]+@[a-z]+\\.[a-z]+$", "Email pattern"),
      ),
    ])
  let ctx = empty_context()
...
pub fn checker_regex_special_chars_test() {
  ...
  let expected =
    make_expected_response([
      #(
        "version",
        Check("string matching ^v[0-9]+\\.[0-9]+\\.[0-9]+$", "Semver"),
      ),
    ])
  let ctx = empty_context()
...
pub fn checker_regex_non_string_value_test() {
  ...
  let expected =
    make_expected_response([
      #("count", Check("string matching ^\\d+$", "Digits")),
    ])
  let ctx = empty_context()
...
pub fn checker_integer_negative_test() {
  ...
  let expected =
    make_expected_response([
      #("offset", Check("integer >= -10", "Valid offset")),
    ])
  let ctx = empty_context()
...
pub fn checker_array_max_items_pass_test() {
  let body =
    json.object([
      #(
        "tags",
        json.array([json.string("a"), json.string("b")], function.identity),
      ),
    ])
  let actual = make_execution_result(body)
  let expected =
    make_expected_response([
      #("tags", Check("array with max 5 items", "Max tags")),
    ])
  let ctx = empty_context()
...
pub fn checker_equals_bool_mismatch_test() {
  ...
  let expected =
    make_expected_response([
      #("active", Check("equals false", "Should be false")),
    ])
  let ctx = empty_context()
