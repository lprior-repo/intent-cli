...
import intent/ai_cue_protocol.{
  AIDirective, AskQuestion, NotStarted,
  create_initial_directive, parse_answer_submission,
  directive_to_cue, validate_session_file,
}
import intent/question_types.{type Question, Question, Critical, Developer}
...
pub fn ai_cue_next_directive_after_answer_test() {
  ...
  // Submit an answer
  let assert Ok(submission) = parse_answer_submission(
    "q-001",
...
pub fn ai_cue_complete_interview_test() {
  ...
  let progress = ai_cue_protocol.Progress(
    current_round: 5,
    total_rounds: 5,
    questions_asked: 24,
    questions_remain: 1,
...
pub fn ai_cue_add_requirement_test() {
  ...
  let req = ai_cue_protocol.EARSRequirement(
    id: "req-001",
