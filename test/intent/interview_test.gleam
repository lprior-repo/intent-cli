...
pub fn detect_gaps_event_profile_test() {
  let gaps = interview.detect_gaps(interview.Event, [])

  let has_event_type_gap =
    list.any(gaps, fn(gap) { string.contains(gap.field, "event_type") })

  has_event_type_gap
  |> should.equal(True)
}

pub fn detect_gaps_data_profile_test() {
  let gaps = interview.detect_gaps(interview.Data, [])

  let has_data_model_gap =
    list.any(gaps, fn(gap) { string.contains(gap.field, "data_model") })

  has_data_model_gap
  |> should.equal(True)
}

pub fn detect_gaps_workflow_profile_test() {
  let gaps = interview.detect_gaps(interview.Workflow, [])

  let has_steps_gap =
    list.any(gaps, fn(gap) { string.contains(gap.field, "steps") })

  has_steps_gap
  |> should.equal(True)
}

pub fn detect_gaps_ui_profile_test() {
  let gaps = interview.detect_gaps(interview.UI, [])

  let has_user_flows_gap =
    list.any(gaps, fn(gap) { string.contains(gap.field, "user_flows") })

  has_user_flows_gap
  |> should.equal(True)
}

pub fn detect_gaps_multiple_missing_fields_test() {
  let gaps = interview.detect_gaps(interview.Api, [])

  // Should have multiple required fields missing (5 for Api profile)
  list.length(gaps)
  |> should.equal(5)
}

pub fn detect_gaps_all_fields_present_test() {
  let answers = [
    interview.Answer(
      question_id: "q1",
      question_text: "Fields",
      perspective: question_types.Developer,
      round: 1,
      response: "All filled",
...
pub fn can_proceed_with_non_blocking_gaps_test() {
  let session = interview.create_session("test", interview.Api, "2024-01-01")
  let gap =
    interview.Gap(
      id: "gap-1",
  ...
  // Non-blocking gaps should allow proceeding
  interview.can_proceed(session)
  |> should.be_ok
}

pub fn gap_tracking_across_rounds_test() {
  let session = interview.create_session("test", interview.Api, "2024-01-01")
  let gap1 =
    interview.Gap(
      id: "gap-1",
      field: "field1",
      description: "Round 1 gap",
      blocking: True,
      suggested_default: "",
      why_needed: "Required",
      round: 1,
  ...
  // Should track gaps from multiple rounds
  list.length(session.gaps)
  |> should.equal(2)
}

pub fn conflict_tracking_through_resolution_test() {
  let session = interview.create_session("test", interview.Api, "2024-01-01")
  let conflict =
    interview.Conflict(
      id: "c1",
  ...
  // Conflict starts unresolved
  let unresolved_before = interview.get_unresolved_conflicts(session)
  list.length(unresolved_before)
  |> should.equal(1)

  // Resolve conflict
  let result = interview.resolve_conflict(session, "c1", 0)
  case result {
    Ok(updated) -> {
      let unresolved_after = interview.get_unresolved_conflicts(updated)
      list.length(unresolved_after)
      |> should.equal(0)
    }
    Error(_) -> should.fail()
  }
...
pub fn multiple_unresolved_conflicts_test() {
  let session = interview.create_session("test", interview.Api, "2024-01-01")
  let c1 =
    interview.Conflict(
      id: "c1",
  ...
  let session = interview.InterviewSession(..session, conflicts: [c1, c2, c3])
...
pub fn resolve_multiple_gaps_test() {
  let session = interview.create_session("test", interview.Api, "2024-01-01")
  let gap1 =
    interview.Gap(
      id: "gap-1",
  ...
  // Resolve first gap
  let session = interview.resolve_gap(session, "gap-1", "Fixed 1")

  // Resolve second gap
  let session = interview.resolve_gap(session, "gap-2", "Fixed 2")

  // Check both are resolved
  let resolved_count = list.count(session.gaps, fn(g) { g.resolved == True })
  resolved_count
  |> should.equal(2)
}
