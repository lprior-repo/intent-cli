...
pub fn analyze_coverage_method_counts_test() {
  ...
  let get_behavior = test_helpers.make_test_behavior_with_method("get-user", Get, [])
  let post_behavior =
...
pub fn analyze_coverage_only_success_statuses_test() {
  ...
  { dict.get(report.status_codes, "4xx") == Ok(0) || dict.get(report.status_codes, "4xx") == Error(Nil) }
  |> should.be_true()
}
...
pub fn analyze_coverage_owasp_keywords_test() {
  ...
  let covered_count = dict.size(report.owasp.categories) - list.length(report.owasp.missing)
...
pub fn format_report_shows_methods_test() {
  ...
  let spec = test_helpers.make_test_spec_from_behaviors([get_behavior, post_behavior])
  ...
  { string.contains(formatted, "Method") || string.contains(formatted, "GET") || string.contains(formatted, "POST") }
  |> should.be_true()
}
...
pub fn format_report_shows_owasp_test() {
  ...
  { string.contains(formatted, "OWASP") || string.contains(formatted, "Security") || string.contains(formatted, "security") }
  |> should.be_true()
}
