...
pub fn analyze_effects_create_behavior_test() {
  ...
  effect_descriptions
  |> list.any(fn(desc) { desc == "Resource can now be retrieved via GET" })
  |> should.be_true()

  effect_descriptions
  |> list.any(fn(desc) { desc == "Resource appears in listing endpoints" })
  |> should.be_true()

  effect_descriptions
  |> list.any(fn(desc) { desc == "Resource count/statistics are updated" })
  |> should.be_true()
...
pub fn analyze_effects_update_behavior_test() {
  ...
  effect_descriptions
  |> list.any(fn(desc) { desc == "Cache entries may need invalidation" })
  |> should.be_true()
...
pub fn analyze_effects_cascade_detection_test() {
  ...
  let spec =
    test_helpers.make_test_spec_from_behaviors([user_delete, team_delete])
