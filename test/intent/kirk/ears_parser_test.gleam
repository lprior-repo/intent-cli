...
pub fn parse_state_driven_with_the_test() {
  ...
  let text = "WHILE processing payment THE SYSTEM SHALL prevent duplicate submissions"
...
pub fn parse_optional_with_the_test() {
  ...
  let text = "WHERE user has premium status THE SYSTEM SHALL enable advanced features"
...
pub fn parse_unwanted_with_then_and_the_test() {
  ...
  let text = "IF request lacks authentication THEN THE SYSTEM SHALL NOT expose sensitive data"
...
pub fn parse_complex_with_the_test() {
  ...
  let text = "WHILE processing transaction WHEN error occurs THE SYSTEM SHALL rollback changes"
...
pub fn parse_complex_without_the_test() {
  ...
  let text = "WHILE user is logged in WHEN session expires SYSTEM SHALL redirect to login"
...
pub fn parse_missing_error_handling_warning_test() {
  ...
  let text = "THE SYSTEM SHALL process request\nTHE SYSTEM SHALL return response"
  ...
  result.warnings
  |> list.any(fn(w) {
    string.contains(string.lowercase(w), "error")
  })
  |> should.be_true()
}
