...
pub fn analyze_spec_with_security_tests_test() {
  let spec = spec_with_behavior("auth-bypass-test", "Test auth bypass returns 401", 401)
  let report = inversion_checker.analyze_inversions(spec)
...
pub fn analyze_spec_with_multiple_tests_test() {
  ...
  let behavior2 = make_behavior("invalid-credentials", "Invalid credentials return 401", 401)
  ...
  let total_gaps = list.length(report.security_gaps)
...
pub fn security_gap_covered_by_anti_pattern_test() {
  let anti_pattern = types.AntiPattern(
...
pub fn usability_not_found_covered_test() {
  let spec = spec_with_behavior("not-found-test", "Test not found returns 404", 404)
  let report = inversion_checker.analyze_inversions(spec)

  // Should not have not-found gap since we test 404
  let has_not_found_gap = list.any(report.usability_gaps, fn(gap) {
...
pub fn severity_to_string_test() {
  should.equal(inversion_checker.severity_to_string(inversion_checker.Low), "low")
  should.equal(inversion_checker.severity_to_string(inversion_checker.Medium), "medium")
  should.equal(inversion_checker.severity_to_string(inversion_checker.High), "high")
  should.equal(inversion_checker.severity_to_string(inversion_checker.Critical), "critical")
}
...
pub fn format_report_with_covered_categories_test() {
  let spec = spec_with_behavior("invalid-credentials", "Invalid credentials return 401", 401)
  let report = inversion_checker.analyze_inversions(spec)
  let formatted = inversion_checker.format_report(report)
