...
import intent/types.{type Behavior, Behavior, Delete, Get, Post, Put}
import test_helpers
...
pub fn detect_gaps_missing_error_codes_test() {
  ...
  {
    string.contains(gap_descriptions, "401")
    || string.contains(gap_descriptions, "unauthorized")
  }
  |> should.be_true()
}
...
pub fn detect_gaps_no_auth_keywords_test() {
  ...
  let has_critical_security_gap =
    ...
    |> list.any(fn(gap) {
      gap.severity == gap_detector.Critical || gap.severity == gap_detector.High
    })
...
pub fn detect_gaps_missing_intents_test() {
  ...
  let behavior =
    Behavior(..test_helpers.make_test_behavior("test-behavior", []), intent: "")
