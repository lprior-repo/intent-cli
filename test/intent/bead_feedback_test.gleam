...
pub fn feedback_error_write_error_test() {
  let error = bead_feedback.WriteError(path: "/tmp/test.cue", message: "Permission denied")
  case error {
...
pub fn bead_feedback_with_error_test() {
  let error = option.Some(bead_feedback.BeadError(
...
pub fn bead_feedback_blocked_test() {
  let blocked = option.Some(bead_feedback.BlockedReason(
