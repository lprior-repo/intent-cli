...
pub fn interpolate_excessive_variables_test() {
  ...
  let excessive_string = string.repeat("${x} ", 101)
...
// =============================================================================
// Circular Reference Detection Tests (intent-cli-mjq4)
// =============================================================================

pub fn interpolate_same_variable_multiple_times_no_cycle_test() {
  // Using the same variable multiple times in one string is NOT a cycle
  let ctx = interpolate.new_context()
  let ctx = interpolate.set_variable(ctx, "name", json.string("Alice"))

  let result = interpolate.interpolate_string(ctx, "${name} and ${name}")

  result |> should.equal(Ok("Alice and Alice"))
}

pub fn extract_capture_nested_path_with_same_var_name_test() {
  // Accessing nested.path.var is NOT a cycle even if "var" exists as variable
  let ctx = interpolate.new_context()
  let nested =
    json.object([
      #("path", json.object([#("var", json.string("nested_value"))])),
    ])
  let ctx = interpolate.set_variable(ctx, "nested", nested)
  let ctx = interpolate.set_variable(ctx, "var", json.string("top_level"))

  // Should resolve nested.path.var without cycle error
  let result = interpolate.extract_capture(ctx, "nested.path.var")

  case result {
    Ok(value) -> {
      interpolate.json_to_string(value) |> should.equal("nested_value")
    }
    Error(_) -> should.fail()
  }
}

pub fn interpolate_nested_object_field_reuses_var_name_test() {
  // A variable referencing its own nested field should work
  let ctx = interpolate.new_context()
  let user =
    json.object([#("user", json.object([#("name", json.string("Bob"))]))])
  let ctx = interpolate.set_variable(ctx, "user", user)

  // ${user.user.name} accesses nested field, not circular
  let result = interpolate.interpolate_string(ctx, "Name: ${user.user.name}")

  result |> should.equal(Ok("Name: Bob"))
}
