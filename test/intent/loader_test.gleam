...
pub fn spinner_handle_creation_test() {
  ...
  case handle {
    Some(SpinnerHandle(set_text: _, stop: _)) -> should.be_true(True)
    None -> should.fail()
  }
}
...
pub fn spinner_handle_none_test() {
  ...
  let handle: option.Option(loader.SpinnerHandle) = None
...
pub fn load_error_file_not_found_construction_test() {
  let error = FileNotFound("path/to/file.cue")
  case error {
    FileNotFound(path) -> path |> should.equal("path/to/file.cue")
    _ -> should.fail()
  }
}

pub fn load_error_cue_validation_failed_construction_test() {
  let error = CueValidationFailed("spec.cue", 1, "error message")
  case error {
...
pub fn load_error_cue_export_failed_construction_test() {
  let error = CueExportFailed("spec.cue", 2, "export error")
  case error {
...
pub fn load_error_security_error_construction_test() {
  let error = SecurityError("security violation detected")
  case error {
