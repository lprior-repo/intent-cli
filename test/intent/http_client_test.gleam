...
/// Test SSRF protection blocks URLs without scheme (intent-cli-jzm2)
pub fn ssrf_blocks_missing_scheme_test() {
  let config =
    types.Config(base_url: "example.com", timeout_ms: 5000, headers: dict.new())
  ...
  case result {
    Error(http_client.SSRFBlocked(msg)) -> {
      string.contains(msg, "missing scheme") |> should.be_true()
      string.contains(msg, "http://") |> should.be_true()
      string.contains(msg, "https://") |> should.be_true()
    }
    _ -> should.fail()
  }
}

/// Test SSRF protection blocks URLs without hostname (intent-cli-4efl)
pub fn ssrf_blocks_missing_hostname_test() {
  let config =
    types.Config(base_url: "http:///api", timeout_ms: 5000, headers: dict.new())
  ...
  case result {
    Error(http_client.SSRFBlocked(msg)) -> {
      // Check that we got an error about missing hostname
      // The message might be "URL missing hostname" or similar
      let has_hostname = string.contains(msg, "hostname")
      let has_host = string.contains(msg, "host")
      case has_hostname || has_host {
        True -> Nil
        False ->
          panic as { "Expected error about missing hostname, but got: " <> msg }
      }
    }
    Error(http_client.UrlParseError(msg)) -> {
      // URL parse might fail for malformed URLs
      panic as { "Got UrlParseError instead of SSRFBlocked: " <> msg }
    }
    Ok(_) -> panic as { "Expected SSRFBlocked error but request succeeded" }
    _ ->
      panic as {
        "Expected SSRFBlocked error but got different error: "
