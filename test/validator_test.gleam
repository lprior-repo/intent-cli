...
fn make_test_behavior(name: String, requires: List(String)) -> Behavior {
  Behavior(
    name: name,
...
pub fn validator_valid_capture_reference_test() {
  ...
  let a = make_behavior_with_captures("create-user", [], [#("user_id", "$.id")])
  let b =
    make_behavior_with_path("get-user", "/users/${user_id}", ["create-user"])
...
pub fn validator_missing_capture_in_header_test() {
  ...
  let b =
    make_behavior_with_headers("get-profile", ["login"], [
...
pub fn validator_multiple_variables_in_path_test() {
  ...
  let a = make_behavior_with_captures("setup", [], [#("org_id", "$.org")])
  let b =
...
pub fn validator_format_missing_capture_with_hint_test() {
  let issues = [
    MissingCapture("get-user", "request.path", "user_id", [
      "create-user",
      "register-user",
    ]),
  ]
...
pub fn validator_complex_capture_chain_test() {
  ...
  let c =
    make_behavior_with_path("get-user", "/orgs/${org_id}/users/${user_id}", [
      "create-user",
    ])
...
pub fn validator_duplicate_capture_names_test() {
  ...
  let b =
    make_behavior_with_captures("create-b", ["create-a"], [#("id", "$.id")])
...
pub fn validator_consecutive_variables_test() {
  ...
  let b =
    make_behavior_with_path("concat", "/items/${prefix}${suffix}", ["setup"])
