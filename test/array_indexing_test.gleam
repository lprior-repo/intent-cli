...
pub fn navigate_nested_field_test() {
  let json_data =
    json.object([
      #(
        "user",
        json.object([#("profile", json.object([#("name", json.string("Bob"))]))]),
      ),
    ])
  ...
  let json_data =
    json.object([
      #(
        "items",
        json.array(
          [json.int(10), json.int(20), json.int(30)],
          function.identity,
        ),
      ),
    ])
...
pub fn navigate_array_out_of_bounds_positive_test() {
  let json_data =
    json.object([
      #(
        "items",
        json.array([json.int(1), json.int(2), json.int(3)], function.identity),
      ),
    ])
...
pub fn navigate_wildcard_error_test() {
  let json_data =
    json.object([
      #("items", json.array([json.int(1), json.int(2)], function.identity)),
    ])
...
pub fn navigate_deeply_nested_not_found_test() {
  let json_data =
    json.object([
      #("a", json.object([#("b", json.object([#("c", json.int(1))]))])),
    ])
            ...
            json.object([
              ...
              #(
                ...
                json.array(
                  [
                    json.object([
                      #("addr", json.string("a@x.com")),
                      #("verified", json.bool(True)),
                    ]),
                    json.object([
                      #("addr", json.string("b@x.com")),
...
pub fn navigate_large_array_last_element_test() {
  ...
  let json_data =
    json.object([#("numbers", json.array(elements, function.identity))])
