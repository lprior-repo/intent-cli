...
import intent/bead_cue.{
  type Bead, Bead, TestSpec, Expectation,
  bead_to_cue, beads_to_cue, generate_bead_id,
}
...
pub fn bead_cue_done_when_test() {
  let bead = Bead(
    ..make_test_bead("AUTH-001", "Implement login", "5min"),
    done_when: ["Login endpoint returns JWT", "Tests pass", "Docs updated"],
  )
  let cue = bead_to_cue(bead)
...
pub fn bead_cue_edge_cases_test() {
  let bead = Bead(
    ..make_test_bead("AUTH-001", "Implement login", "5min"),
    edge_cases: ["Invalid credentials", "Expired token", "Missing password"],
  )
  let cue = bead_to_cue(bead)
