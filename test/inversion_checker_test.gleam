...
import intent/kirk/inversion_checker.{Critical, High, analyze_inversions}
import intent/types.{
  type AntiPattern, type Behavior, type Rule, type Spec, AIHints, AntiPattern,
  Behavior, Config, Feature, ImplementationHints, Request, Response, Rule,
  RuleCheck, SecurityHints, Spec, When,
}
...
fn make_test_spec(behaviors: List(Behavior)) -> Spec {
  let feature =
    Feature(name: "Test", description: "Test feature", behaviors: behaviors)
...
fn make_rule(name: String) -> Rule {
  Rule(
    ...
    when: When(status: option.Some("*"), method: option.Some(types.Get), path: option.Some("*")),
    check: RuleCheck(
...
pub fn inversion_sql_injection_critical_severity_test() {
  ...
  report.security_gaps
  |> list.any(fn(gap) {
    string.contains(gap.description, "SQL injection")
...
pub fn inversion_suggestions_use_existing_paths_test() {
  ...
  let b =
    make_test_behavior("get-user", "Get user", 200, types.Get, "/users/123")
  let spec = make_test_spec([b])
  let report = analyze_inversions(spec)
...
pub fn inversion_analyzes_all_features_test() {
  ...
  let f1 =
    Feature(name: "Auth", description: "Auth feature", behaviors: [
      make_test_behavior(
        "auth-bypass-test",
        "auth bypass",
        401,
  ...
  let f2 =
    Feature(name: "Users", description: "Users feature", behaviors: [
      make_test_behavior(
        "not-found-test",
        "not found",
        404,
  ...
  let spec = Spec(..make_test_spec([]), features: [f1, f2])
...
pub fn inversion_case_insensitive_name_match_test() {
  ...
  let b =
    make_test_behavior("AUTH-BYPASS-TEST", "Test auth", 401, types.Get, "/api")
  let spec = make_test_spec([b])
  let report = analyze_inversions(spec)
