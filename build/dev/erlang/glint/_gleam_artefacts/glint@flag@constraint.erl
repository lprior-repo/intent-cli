...
-spec one_of(list(KDE)) -> fun((KDE) -> {ok, nil} | {error, snag:snag()}).
one_of(Allowed) ->
...
-spec none_of(list(KDH)) -> fun((KDH) -> {ok, nil} | {error, snag:snag()}).
none_of(Disallowed) ->
...
-spec each(fun((KDK) -> {ok, nil} | {error, snag:snag()})) -> fun((list(KDK)) -> {ok,
        nil} |
    {error, snag:snag()}).
each(Constraint) ->
    fun(L) -> _pipe = L,
