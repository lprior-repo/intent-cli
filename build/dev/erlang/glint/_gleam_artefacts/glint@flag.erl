...
-opaque flag_builder(IJS) :: {flag_builder,
        binary(),
        fun((binary()) -> {ok, IJS} | {error, snag:snag()}),
        fun((internal(IJS)) -> value()),
        gleam@option:option(IJS)}.

-opaque internal(IJT) :: {internal,
        gleam@option:option(IJT),
        fun((binary()) -> {ok, IJT} | {error, snag:snag()})}.
...
-spec new(
    fun((internal(IKK)) -> value()),
    fun((binary()) -> {ok, IKK} | {error, snag:snag()})
) -> flag_builder(IKK).
new(Valuer, P) ->
...
-spec wrap_with_constraint(
    fun((binary()) -> {ok, IKV} | {error, snag:snag()}),
    fun((IKV) -> {ok, nil} | {error, snag:snag()})
) -> fun((binary()) -> {ok, IKV} | {error, snag:snag()}).
...
-spec constraint(
    flag_builder(IKR),
    fun((IKR) -> {ok, nil} | {error, snag:snag()})
) -> flag_builder(IKR).
...
-spec description(flag_builder(ILK), binary()) -> flag_builder(ILK).
...
-spec default(flag_builder(ILN), ILN) -> flag_builder(ILN).
...
-spec construct_value(binary(), internal(ILX), fun((internal(ILX)) -> value())) -> {ok,
        value()} |
    {error, snag:snag()}.
...
-spec get_value(
    gleam@dict:dict(binary(), flag()),
    binary(),
    fun((flag()) -> {ok, IMD} | {error, snag:snag()})
) -> {ok, IMD} | {error, snag:snag()}.
